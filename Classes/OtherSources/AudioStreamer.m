...
- (void)failWithErrorCode:(AudioStreamerErrorCode)anErrorCode
{
	@synchronized(self)
	{
		...
		NSString* str = [AudioStreamer stringForErrorCode:anErrorCode];
		NSString *errorStr = [NSString  stringWithFormat:@"Unable to configure network read stream.Error:%@", str];

		[delegate error:[AudioStreamer stringForErrorCode:anErrorCode]];
		/*
	...
	@synchronized(self)
	{
		...
		if (fileLength > 0 && seekByteOffset > 0)
		{
			CFHTTPMessageSetHeaderFieldValue(message, CFSTR("Range"),
				(CFStringRef)[NSString stringWithFormat:@"bytes=%ld-%ld", seekByteOffset, (long)fileLength]);
			discontinuous = YES;
		}
